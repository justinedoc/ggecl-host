...
import AuthDivider from "../ui/AuthDivider.tsx";
import GoogleSigninBtn from "../ui/GoogleSigninBtn.tsx";
...
import { InstructorLoginResponseSchema } from "@/schemas/instructorResponse.ts";
...
const InstructorLogin = () => {
  ...
  const { isPending, mutate } = useMutation({
    mutationFn: loginInstructor,
    onSuccess: (data) => {
      try {
        const validatedData = InstructorLoginResponseSchema.parse(data.data);
    ...
    <AuthContainer isPending={isPending}>
      <div className="flex flex-col gap-4 w-full md:mt-5 md:p-5 relative">
        <div className="w-full space-y-5">
          <FormHead title="Instructor Login">
            Login to your instructor account to teaching
          </FormHead>
